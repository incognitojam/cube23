...
case "$EXECUTION_MODE" in
    ...
    "mac_x11"|"ci"|"docker_headless"|*)
        ...
        run "cd build && timeout 10s bash -c 'CUBE23_TEST_MODE=1 LIBGL_ALWAYS_SOFTWARE=1 XDG_RUNTIME_DIR=/tmp xvfb-run -a --server-args=\"-screen 0 800x600x24\" bash -c \"./cube23 &
        sleep 2
